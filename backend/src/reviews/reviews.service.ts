...
export class ReviewsService {
    ...
    async findByOfferId(offerId: string): Promise<Review[]> {
        return this.prisma.review.findMany({
            where: { offerId },
            include: { author: { select: { id: true, displayName: true, avatarUrl: true, tier: true } } },
    ...
    async getOfferStats(offerId: string): Promise<{ averageRating: number; totalReviews: number }> {
